...
public interface iOradorDAO {
	...
	// SE DEBE IMPLEMENTAR EN UNA CLASE, ES DECIR EN UNA CLASE USAMOS ESTOS METODOS
	// QUE DEFINIMOS ACA

	/*
	 * crud getById() find() delete() update() create()
	 */
	...
	// delete from oradores where id_orador = id;pero primero buscador el orador si
	// existe en la base por su mail y el id levantandolo de la base
	public void deleteOrador(Orador orador) throws Exception;// esto borra un registro por el id del oradores

	// update oradores set nombre = nombre, apellido = apellido, mail = mail, tema =
	// tema , activo = activo where id_orador = orador.id;
	public void update(Orador orador) throws Exception; // se le pasa un objeto

	// insert to oradores (campo 1..campo2..campo3)
	// values(newOrador.campo1....newOrador.campoN)
	...
	// crear un method que consulte a la base por el nombre y que devuelva una lista
	// de departamentos

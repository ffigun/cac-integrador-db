...
//http://localhost:8080/integrador/BuscarOradorPorTemaController
@WebServlet("/BuscarOradorPorTemaController")

public class BuscarOradorPorTemaController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			oradores = dao.search(tema);
		} catch (Exception e) {
			e.printStackTrace(); // muestra por consola el error
		}
		req.setAttribute("listado", oradores);
		// este bloque de codigo lo vamos a usar en todos lados, redirecciona al
		// index.jsp
